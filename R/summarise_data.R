...
summarise_anc_data_univariate <- function(anc_data_processed_subset_2023,
                                          simplify = TRUE) {
  summary_tab <- list(
    `Age` = anc_data_processed_subset_2023 |>
      dplyr::summarise(
        `Median Age` = median(age, na.rm = TRUE)
      ) |>
      tidyr::pivot_longer(
      ...
      dplyr::mutate(value = paste0(value1, " (", value2, ")")) |>
      dplyr::select(var, value),
    `Age Group` = anc_data_processed_subset_2023 |>
      dplyr::count(age_group) |>
      #dplyr::arrange(dplyr::desc(n)) |>
      dplyr::mutate(
        n = paste0(
      ...
      dplyr::rename(var = age_group, value = n),
    `Haemoglobin` = anc_data_processed_subset_2023 |>
      dplyr::summarise(
        `Median Hb` = median(haemoglobin, na.rm = TRUE)
      ) |>
      tidyr::pivot_longer(
      ...
      dplyr::mutate(value = paste0(value1, " (", value2, ")")) |>
      dplyr::select(var, value),
    `Anaemia Status` = anc_data_processed_subset_2023 |>
      dplyr::count(anaemia_status) |>
      dplyr::arrange(dplyr::desc(n)) |>
      dplyr::mutate(
        n = paste0(
          n, " (", 
          scales::label_percent()(n / unique(anc_data_processed_subset_2023$total_n)),
          ")"
        )
      ) |>
      dplyr::rename(var = anaemia_status, value = n),
    `Anaemia Category` = anc_data_processed_subset_2023 |>
      dplyr::count(anaemia_category) |>
      #dplyr::arrange(dplyr::desc(n)) |>
      dplyr::mutate(
        n = paste0(
      ...
      dplyr::rename(var = anaemia_category, value = n),
    Profession = anc_data_processed_subset_2023 |>
      ...
      dplyr::rename(var = profession_summary, value = n),
    `Education Level` = anc_data_processed_subset_2023 |>
      dplyr::count(profession) |>
      dplyr::arrange(dplyr::desc(n)) |>
      dplyr::mutate(
        n = paste0(
  ...
  `Education Level` = anc_data_processed_subset_2023 |>
    ...
    dplyr::rename(var = education_level_summary, value = n),
  `Marital Status` = anc_data_processed_subset_2023 |>
    dplyr::count(education_level) |>
    dplyr::arrange(dplyr::desc(n)) |>
    dplyr::mutate(
      n = paste0(
    ...
    `Marital Status` = anc_data_processed_subset_2023 |>
      ...
      dplyr::rename(var = marital_status, value = n),
    `Address` = anc_data_processed_subset_2023 |>
      dplyr::count(address) |>
      dplyr::arrange(dplyr::desc(n)) |>
      dplyr::mutate(
        n = paste0(
      ...
      dplyr::rename(var = address, value = n),
    `Sickle Cell` = anc_data_processed_subset_2023 |>
      dplyr::count(sickle_cell) |>
      dplyr::arrange(dplyr::desc(n)) |>
      dplyr::mutate(
        n = paste0(
      ...
      dplyr::rename(var = sickle_cell, value = n),
    `Malaria` = anc_data_processed_subset_2023 |>
      dplyr::count(malaria) |>
      dplyr::arrange(dplyr::desc(n)) |>
      dplyr::mutate(
        n = paste0(
...
summarise_bivariate_continuous_2023 <- function(df, 
                                           outcome, 
                                           predictor,
                                           predictor_label = predictor) {
  df |>
...
summarise_bivariate_categorical_2023 <- function(df,
                                            outcome,
                                            predictor,
                                            predictor_label = predictor) {
  df_outcome_total <- df |>
...
summarise_bivariate_2023 <- function(df,
                                outcome,
                                predictor, 
                                predictor_label = predictor,
                                predictor_type = c("continuous", "categorical")) {
