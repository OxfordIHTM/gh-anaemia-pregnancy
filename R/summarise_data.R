...
summarise_anc_data_bivariate <- function(anc_data_recode,
  ...
  summary_tab <- list(
    `Age` = anc_data_recode |>
      dplyr::group_by(anaemia_status) |>
      dplyr::summarise(
        `Median Age` = median(age, na.rm = TRUE), 
        `Confidence Interval` = boxplot.stats(age)$conf |>
          round(digits = 1) |>
          paste(collapse = ", "),
        .groups = "drop"
      ) |>
      tidyr::pivot_wider(
        names_from = anaemia_status, 
        values_from = `Median Age`:`Confidence Interval`
      )
      
      
      dplyr::mutate(conf_int = boxplot.stats())
       |>
      cbind(
        anc_data_recode |>
          dplyr::summarise(
